    ...
    <div class="detail-wrapper">
        <div class="detail-container">
            ...
            @if ($work->isPending() && auth()->user()->isAdmin() && $fromApplication)
                <form class="work-form" method="POST"
                    action="{{ route('admin.application.approve', ['id' => $application->id]) }}">
                    @csrf
        ...
        ================================= --}}
            ...
            @if (!$work->hasApplication() && !auth()->user()->isAdmin())
                <form class="work-form" method="POST" action="{{ route('attendance.application.store', $work->id) }}">
                    @csrf
            ...
            ================================ --}}
            @if (!$work->hasApplication() && auth()->user()->isAdmin())
                <form class="work-form" method="POST" action="{{ route('admin.update', $work->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="edit-table">
                        @include('works.partials._works_fields', ['work' => $work, 'breaks' => $breaks])
